...
type UpdateFormProps = {
 ...
 const ProfileUpdateForm: React.FC<UpdateFormProps> = ({ isActive }) => {
  ...
  const onFinish: FormProps<UpdateFormData>['onFinish'] = async (values: UpdateFormData) => {
    try {
      // if (user) values.id = user.id;

      console.log('Form values:', values); // Проверка значений формы
      // console.log('Avatar file:', values.avatar); // Проверка файла аватара
      ...
      if (values.avatar) {
        formData.append('avatar', values.avatar.fileList[0].originFileObj as File); // Убедитесь, что имя поля совпадает
      }
  ...
  <Upload 
    ...
    onChange={(info) => {
      console.log(info.fileList); // Проверка загруженных файлов
