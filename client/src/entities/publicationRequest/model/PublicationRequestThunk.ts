...
>("publicationRequest/createPublicationRequest", async ({lyricFileId}, { rejectWithValue }) => {
    try {
        return await PublicationRequestService.createPublicationRequest(lyricFileId);
    } catch (error) {
...
export const getAllPublicationRequests = createAsyncThunk<
    ...
    void,
    { rejectValue: RejectValue }
>("publicationRequest/getAllPublicationRequests", async (_, { rejectWithValue }) => {
    try {
...
export const deletePublicationRequest = createAsyncThunk<
    ...
    { rejectValue: RejectValue }
>("publicationRequest/deletePublicationRequest", async ({publicationRequestId}, { rejectWithValue }) => {
    try {
        return await PublicationRequestService.deletePublicationRequest(publicationRequestId);
    } catch (error) {
        const err = error as AxiosError<{ message: string }>;
        return rejectWithValue({
            message: err.response?.data.message || err.message,
